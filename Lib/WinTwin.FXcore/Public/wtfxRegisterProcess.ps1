function wtfxRegisterProcess {
    ...
    try {
        ...
        # 'pid' may not exist yet on older process.json files - Add-Member with
        # -Force both creates it on first use and updates it on every call after.
        $db.running | Add-Member -MemberType NoteProperty -Name 'pid' -Value $ProcessId -Force
    }
    catch {
