...
**WinTwin.FXcore** is a PowerShell module that provides a complete, fully automatable
pipeline for:
...
Copy-Item -Path '.\PowerShell.Mods\WinTwin.FXcore' `
          -Destination "$env:ProgramFiles\WindowsPowerShell\Modules\WinTwin.FXcore" `
          -Recurse -Force

# 3. Import
Import-Module WinTwin.FXcore -Verbose
...
```
WinTwin.FXcore\
├── WinTwin.FXcore.psm1        ← Module root: scope initialisation + dot-source loader
├── WinTwin.FXcore.psd1        ← Module manifest (version, exports, file list)
