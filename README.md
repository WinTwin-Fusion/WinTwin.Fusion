...
| **Core** | Shared framework runtime: `config.json`, logging, language files, UI resources, fonts, PID/state store. | In progress |
...
management and offline registry hive editing. The console-only installer (`wintwin.installer.ps1`)
is available and functional. The DISM.UI.CC GUI tools, USMT.Composer and PS.Tweak.Tools are in
active planning/early development. See `CHANGELOG.md` for a detailed version history and
`DEV.TASKS.md` for the current task backlog.
...
`WinTwin.FXcore`, `PSAppCoreLib` and (optionally) `VPDLX` modules where applicable, per the
project's internal architecture guidelines.
